...
<table style="margin-top:10px;" width="999" align="center" cellpadding="0" cellspacing="0" class="thin-border">
	<tr>
		<td align="center" bgcolor="#FFB6C1">商品ID</td>
		<td align="center" bgcolor="#FFB6C1">商品数量</td>
		<td align="center" bgcolor="#FFB6C1">商品价格</td>
		<td align="center" bgcolor="#FFB6C1">操作</td>
	</tr>
	<c:if test="${empty shopCart || shopCart.isEmpty }">
		<tr>
			<td colspan="4">购物车中没有任何商品，请<a href="product.do?method=list">选择商品</a></td>
		</tr>
	</c:if>
	<c:if test="${not empty shopCart && not shopCart.isEmpty }">
		...
		<tr><td colspan="4" align="center">选择收货地址</td></tr>
		<c:if test="${empty addresses }">
			<tr>
				<td colspan="4">
				还没有任何的收货地址<a href="address.do?method=addInput&userId=${loginUser.id }">添加收货地址</a>
				</td>
			</tr>
		</c:if>
				...
				<td colspan="4">
					<c:forEach items="${addresses}" var="address" varStatus="vs">
						<c:if test="${vs.index eq 0 }">
							<input type="radio" name="address" checked="checked" value="${address.id }"/>${address.name }[${address.phone }]<br/>
						</c:if>
						<c:if test="${vs.index ne 0 }">
							<input type="radio" name="address" value="${address.id }"/>${address.name }[${address.phone }]<br/>
						</c:if>
					</c:forEach>	
				</td>
			</tr>
