...
<body>
	<table style="margin-top: 10px;" width="999" align="center"
		cellpadding="0" cellspacing="0" class="thin-border">
		<tr>
			<td colspan="5">
				<form action="orders.do">
					<input type="hidden" name="method" value="list" />
					 <select name="status">
						<option>选择状态</option>
						<option value="1" <c:if test="${param.status eq 1}">selected="selected"</c:if>>已下单</option>
						...
						<option value="4" <c:if test="${param.status eq 4}">selected="selected"</c:if>>已确认
						</option>
					</select> 
		...
		<c:forEach items="${orders.datas }" var="o">
			<tr>
				...
				<td>
					<c:if test="${o.status eq 1 }">已下单</c:if> 
					<c:if test="${o.status eq 2 }">已付款</c:if> 
					<c:if test="${o.status eq 3 }">已发货</c:if>
					<c:if test="${o.status eq 4 }">已确认</c:if>
				</td>
				<td>${o.user.nickname}&nbsp;<a href="orders.do?method=userList&id=${o.user.id }">查询用户订单</a>
				</td>
				<td>
